...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for scene_name in scene_names:
            ...
            try:
                i = 0
                while np.linalg.norm(car_positions[i]-car_positions[0])==0:
                    i+=1
            except:
                return self.__getitem__(index)
            ...
            # print(car_positions)
            # plt.figure()
            # plt.scatter(car_positions[:,0], car_positions[:,1], label='before')

            v = car_positions[i]-car_positions[0]
            if v[1] > 0:
            ...
            for i in range(len(car_positions)):
                car_positions[i] = (M @ (car_positions[i]-car_positions[0]).reshape((-1,1))).flatten() + car_positions[0]
            
            # print(car_positions)
            # plt.scatter(car_positions[:,0], car_positions[:,1], label='after')
            # plt.legend()
            # plt.savefig('y.png')
            # exit()
