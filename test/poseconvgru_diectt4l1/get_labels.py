    ...
    def __getitem__(self, index):
        ...
        for scene_name in scene_names:
            ...
            for i in range(len(measurements)):
                car_positions.append(np.array((measurements[i]['x'], measurements[i]['y'])))
            
            car_positions = np.array(car_positions)

            rel_translations = []
            for i in range(len(car_positions)-1,0,-1):
                rel_translations.append(car_positions[i]-car_positions[i-1])
            rel_translations = list(reversed(rel_translations))
            ...
            imageio.imwrite(f'/mnt/qb/work/geiger/pghosh58/transfuser/vis/scenes/{index}scenet.jpg', scenes[0])  #write all changed images
            ...
            return torch.cat([torch.cat([scenes[i-1], scenes[i]])[None,:,:,:] for i in range(1,len(scenes))]), torch.tensor(rel_translations, dtype=torch.float)
...
gt_wp = [np.array([0.0,0.0])]
pred_wp = [np.array([0.0,0.0])]
start_index = 50
with torch.no_grad():
    for i in range(start_index, start_index+50):#len(test_dataset.driving_scenes)):
        (scenes, rel_translation) = test_dataset[i]
        ...
        if scenes is not None:
            ...
            #forward pass
            pred_rel_translation, z = model(scenes, z)
            print(pred_rel_translation)
            print(rel_translation)
            # plt.figure()
            ...
            gt_wp.append(gt_wp[-1] + np.array(rel_translation[0]))
            pred_wp.append(pred_wp[-1] + pred_rel_translation[0].cpu().numpy())
        else:
            break


gt_wp = np.array(gt_wp)
...
plt.figure()
plt.plot(gt_wp[:,0], gt_wp[:,1], alpha=0.5, marker='.', label='gt_wp')
