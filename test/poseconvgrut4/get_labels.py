...
class RelNet(nn.Module):
    ...
    def forward(self, seq, z):
        ...
        z = self.decoder(x, z)
        dx = self.regressor(z) * (torch.sigmoid(torch.clip(self.classifier(z),-3,3))>0.5)
        return dx, z
...
with torch.no_grad():
    for i in range(len(train_dataset.driving_scenes)):
        (scenes, rel_motions, is_start) = train_dataset[i]
        ...
        # print(rel_motions)
        rel.append(rel_motions[0])
        rel_pred.append(pred_rel_motions[0].cpu())
...
for p in rel:
    if len(pred_wp)==0:
        ...
        rel1 = abs_wp[-1] - abs0
        rel2 = p
    pred_wp.append(abs0, rel1, rel2)
pred_wp = np.array(pred_wp)

plt.plot(abs_wp[:,0], abs_wp[:,1])
plt.plot(pred_wp[:,0], pred_wp[:,1])
