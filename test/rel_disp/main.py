...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.driving_scenes_dict.get(scene_tp1, False):
            ...
            scene_t = torch.from_numpy(np.array(scale_and_crop_image(Image.open(scene_t), scale=1, crop=256)))
            scene_tp1 = torch.from_numpy(np.array(scale_and_crop_image(Image.open(scene_tp1), scale=1, crop=256)))

            return scene_t.float(), scene_tp1.float(), torch.tensor([x_rel, y_rel], dtype=torch.float)
...
class RelNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.pred_rel_motion = nn.Sequential(
                            nn.Linear(512, 256),
    ...
    def forward(self,scenes_t, scenes_tp1):
        enc_t = self.features(normalize_imagenet(scenes_t))
        enc_tp1 = self.features(normalize_imagenet(scenes_tp1))
        enc = enc_tp1-enc_t
        pred_rel_motions = self.pred_rel_motion(enc)
        return pred_rel_motions
...
for epoch in range(num_epochs):
    for i, (scenes_t, scenes_tp1, rel_motions) in enumerate(train_loader):
        scenes_t = scenes_t.to(device)
        scenes_tp1 = scenes_tp1.to(device)
        rel_motions = rel_motions.to(device)

        #forward pass
        pred_rel_motions = model(scenes_t, scenes_tp1)
    ...
    # test
            with torch.no_grad():
                cum_loss = 0
                for i, (scenes_t, scenes_tp1, rel_motions) in enumerate(test_loader):
                    scenes_t = scenes_t.to(device)
                    scenes_tp1 = scenes_tp1.to(device)
                    rel_motions = rel_motions.to(device)
