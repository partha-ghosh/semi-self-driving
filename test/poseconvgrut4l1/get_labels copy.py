    ...
    def __getitem__(self, index):
        ...
        for scene_name in scene_names:
            ...
            for i in range(2, len(measurements)):
                x_proj, y_proj = get_rel_pos(
                    np.array([measurements[i-2]['x'],measurements[i-2]['y']]),
                    np.array([measurements[i-1]['x'],measurements[i-1]['y']]),
                    np.array([measurements[i]['x'],measurements[i]['y']]))
            ...
            return torch.cat([torch.cat([scenes[i-1], scenes[i]])[None,:,:,:] for i in range(1,len(scenes))]), torch.tensor(rel_translations, dtype=torch.float), abs_pos
...
def get_abs_pos(abs0, abs1, rel2=None):
    ...
    if np.linalg.norm(rel2) < 0.001:
        return abs0+1.001*(abs1-abs0)/np.linalg.norm(abs1-abs0)
    else:
        rel1 = abs1 - abs0
...
with torch.no_grad():
    for i in range(start_index,start_index+50):#len(test_dataset.driving_scenes)):
        (scenes, rel_motions, ap) = test_dataset[i]
        ...
        if scenes is not None:
            ...
            print(rel_motions)
            plt.figure()
            plt.scatter([-4,-4,4,4],[-4,4,-4,4])
            plt.scatter(rel_motions[:,0], rel_motions[:,1], label='gt')
            plt.scatter(pred_rel_motions[:,0].cpu().numpy(), pred_rel_motions[:,1].cpu().numpy(),label='pred')
            plt.legend()
            plt.savefig(f'img/{i}.png')
            rel.append(np.array(rel_motions[0]))
            rel_pred.append(pred_rel_motions[0].cpu().numpy())
...
abs_pos = np.array(abs_pos[1:])
abs_pos = abs_pos - abs_pos[0]

abs_wp = [np.array([0.0,0.0]), rel[0]]
pred_wp = [np.array([0.0,0.0]), rel_pred[0]]

print("Check if rel and rel_pred same len", len(rel) == len(rel_pred))

for i in range(1,len(rel)):
    abs_wp.append(get_abs_pos(abs_wp[-2], abs_wp[-1], rel[i]))
    pred_wp.append(get_abs_pos(pred_wp[-2], pred_wp[-1], rel_pred[i]))

    abs_wp2 = np.array(abs_wp)
    pred_wp2 = np.array(pred_wp)
    plt.figure()
    plt.plot(abs_wp2[:,0], abs_wp2[:,1], alpha=0.5, linewidth=2, label='abs_wp')
    plt.plot(pred_wp2[:,0], pred_wp2[:,1], alpha=0.5, label='pred_wp')
    plt.plot(abs_pos[:i+1,0], abs_pos[:i+1,1], alpha=0.5, label='abs_pos')
    plt.legend()
    plt.savefig(f'img2/{i}.png')
...
plt.plot(pred_wp[:,0], pred_wp[:,1], alpha=0.5, marker='.', label='pred_wp')
