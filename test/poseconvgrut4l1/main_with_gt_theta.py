    ...
    def __getitem__(self, index):
        ...
        for scene_name in scene_names:
            ...
            for measurement in measurements:
                ...
                abs_theta.append(measurement['theta'])
            ...
            plt.plot(np.round(abs_pos[:,0],2), np.round(abs_pos[:,1],2), marker='.',label='gt')
            
            for i in range(1, len(abs_pos)):
                ax = np.array([np.cos(abs_theta[i]-abs_theta[i-1]), np.sin(abs_theta[i]-abs_theta[i-1])])
                v = abs_pos[i]-abs_pos[i-1]
                rel_x = v.dot(ax)
                ay = v-(ax*rel_x)
                rel_y = np.linalg.norm(ay) if np.cross(ax, ay)>0 else -np.linalg.norm(ay)
                rel_translations.append([rel_x, rel_y, *ax])
            ...
            # M = np.array([[np.cos(abs_theta[0]), -np.sin(abs_theta[0])], [np.sin(abs_theta[0]), np.cos(abs_theta[0])]])
            # pred_abs_pos = (M@(pred_abs_pos-abs_pos[0]).T).T+abs_pos[0]
            # print(pred_abs_pos)
            # plt.plot(pred_abs_pos[:,0],pred_abs_pos[:,1],marker='.',label='pred_after',alpha=0.5,linewidth=2)
            
            plt.legend()
            plt.savefig('x.png')

            for i in range(len(scenes)):
                imageio.imwrite(f'/mnt/qb/work/geiger/pghosh58/transfuser/vis/scenes/{i}scene.jpg', scenes[i])  #write all changed images
            ...
            return torch.cat([torch.cat([scenes[i-1], scenes[i]])[None,:,:,:] for i in range(1,n_seqs)]), torch.tensor(rel_translations[:n_seqs], dtype=torch.float)
...
for epoch in range(num_epochs):
    for i, (scenes, rel_motions) in enumerate(train_loader):
        ...
        print(scenes.shape, rel_motions.shape)
        exit()
