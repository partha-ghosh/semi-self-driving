...
from ssd.aim_nav.model import AIM
from ssd.aim_nav.config import GlobalConfig
from ssd.aim_nav.data import scale_and_crop_image
...
class AIMAgent(autonomous_agent.AutonomousAgent):
	...
	def run_step(self, input_data, timestamp):
		...
		c = torch.zeros(6).to('cuda')
		c[int(command.item()-1)] = 1

		pred_wp = self.net(encoding, gt_velocity, c)
