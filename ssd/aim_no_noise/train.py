...
if config.train_with_ssd_data:
	...
	train_set = CARLA_Data(root=(config.ssd_train_data+config.train_data), config=config, is_imgaug=False)
	dataloader_train = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=8, pin_memory=True)
	for epoch in range(trainer.cur_epoch, args.epochs): 
		trainer.train()
		if epoch % args.val_every == 0: 
...
if not config.train_with_ssd_data:
	...
	train_set = CARLA_Data(root=config.train_data, config=config, is_imgaug=False)
	dataloader_train = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=8, pin_memory=True)
	for epoch in range(trainer.cur_epoch, args.epochs): 
		trainer.train()
		if epoch % args.val_every == 0: 
