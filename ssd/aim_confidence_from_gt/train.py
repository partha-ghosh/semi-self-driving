	...
	def get_labels(self):
		...
		with torch.no_grad():	
			...
			for data in tqdm(dataloader_ssd):
				...
				gt_waypoints = [torch.stack(data['waypoints'][i], dim=1).to(args.device, dtype=torch.float32) for i in range(config.seq_len, len(data['waypoints']))]
				gt_waypoints = torch.stack(gt_waypoints, dim=1)
				...
				for i in range(len(pred_wp)):
					bbl = 1
					x = bbl - min(bbl, abs(pred_wp[i][len(pred_wp)-1][0] - gt_waypoints[i][len(gt_waypoints)-1][0]))
					y = bbl - min(bbl, abs(pred_wp[i][len(pred_wp)-1][1] - gt_waypoints[i][len(gt_waypoints)-1][1]))
