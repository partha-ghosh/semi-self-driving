...
class Engine(object):
	...
	def get_labels(self):
		model.eval()
		preload_file = os.path.join(config.local_root_dir, 'ssd_data/' + 'rg_cilrs_pl_'+str(config.seq_len)+'_'+str(config.pred_len)+'.npy')
		...
		with torch.no_grad():
			...
			for data in tqdm(dataloader_ssd):
				...
				for i in range(len(steer)):
					...
					preload_steer.append(steer[i].item())
					preload_throttle.append(throttle[i].item())
					preload_brake.append(brake[i].item())
					preload_command.append(command[i].item())
					preload_velocity.append(gt_velocity[i].item())
