...
class AIM(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        self.target_point = nn.Sequential(
                            nn.Linear(16, 8),
                            nn.ReLU(inplace=True),
                            nn.Linear(8, 2),
                        ).to(self.device)
    ...
    def forward(self, feature_emb, velocity, command):
        '''
        ...
        nav_proj = self.proj_nav_command(command)
        vel_proj = self.proj_velocity(velocity.reshape((-1,1)))
        target_point = self.target_point(nav_proj+vel_proj)
