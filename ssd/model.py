...
class ImageCNN(nn.Module):
    ...
    def __init__(self, c_dim, config, normalize=True):
        super().__init__()
        self.normalize = normalize
        self.features = models.resnet34(pretrained=True)
        self.features.fc = nn.Sequential()
        if config['imgcnn']:
            # self.features = vit_small(16, num_classes=0)
            state_dict = torch.load(config['imgcnn'])['teacher']
...
class AIM(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        self.image_encoder = ImageCNN(512, config, normalize=True).to(self.device)
